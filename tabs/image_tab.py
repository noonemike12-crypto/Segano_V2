...
class WorkerThread(QThread):
    ...
    def run(self):
        try:
            logger.log("debug", f"Worker started for: {self.func.__name__}")
            self.progress.emit(30)
            result = self.func(*self.args)
            self.progress.emit(100)
            self.finished.emit(f"✅ {result}")
            logger.log("info", f"Worker finished successfully: {self.func.__name__}")
        except Exception as e:
            logger.log("error", f"Worker failed in {self.func.__name__}: {str(e)}")
...
class ImageTab(QWidget):
    ...
    def browse_image(self):
        logger.log("debug", "Opening image browser...")
        path, _ = QFileDialog.getOpenFileName(self, "Select Image", "", "Images (*.png *.jpg *.bmp)")
        if path:
            logger.log("info", f"Image selected: {path}")
    ...
    def process_hide(self):
        if not self.selected_file:
            logger.log("warning", "Attempted hide without image.")
            return
        logger.log("info", "User triggered 'Hide Message'")
    ...
    def process_extract(self):
        if not self.selected_file:
            logger.log("warning", "Attempted extract without image.")
            return
        logger.log("info", "User triggered 'Extract Message'")
    ...
    def on_finished(self, msg):
        ...
        if "✅" in msg and "extract" in self.sender().func.__name__.lower():
